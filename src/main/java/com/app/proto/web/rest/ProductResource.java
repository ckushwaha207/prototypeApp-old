...
public class ProductResource {
    ...
    /**
     * GET  /products/barcode/:barcodeId : get the "barcodeId" product.
     *
     * @param barcodeId the barcodeId of the productDTO to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the productDTO, or with status 404 (Not Found)
     */
    @GetMapping("/products/barcode/{barcodeId}")
    @Timed
    public ResponseEntity<ProductDTO> getProduct(@PathVariable String barcodeId) {
        log.debug("REST request to get Product using barcode: {}", barcodeId);
        ProductDTO productDTO = productService.findOneByBarcodeId(barcodeId);
        return Optional.ofNullable(productDTO)
            .map(result -> new ResponseEntity<>(
                result,
                HttpStatus.OK))
